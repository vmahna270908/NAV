...
resource "azurerm_network_interface" "AZ-NAV-JMP2-NIC" {
  name                = var.JumpServer2_NIC
  location            = data.azurerm_resource_group.Dev-RG.location
  resource_group_name = data.azurerm_resource_group.Dev-RG.name
...
resource "azurerm_windows_virtual_machine" "AZ-NAV-JMP2" {
  name                = var.JumpServer2_Name
  resource_group_name = data.azurerm_resource_group.Dev-RG.name
  location            = data.azurerm_resource_group.Dev-RG.location
  size                = "Standard_F2"
  admin_username      = "Skywalker"
  admin_password      = "P@$$w0rd1234!"
  network_interface_ids = [
    azurerm_network_interface.AZ-NAV-JMP2-NIC.id,
  ]
